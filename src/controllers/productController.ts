...
export const createProduct = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    // Filter out null/empty/invalid values from images array
...
export const deleteProduct = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    if (product.images && product.images.length > 0) {
      try {
        const { deleteMultipleImages } = await import("./uploadController");
        const deleteResults = await deleteMultipleImages(product.images);

        if (deleteResults.failed > 0) {
          console.warn(
            `Failed to delete ${deleteResults.failed} image(s) for product ${req.params.id}:`,
